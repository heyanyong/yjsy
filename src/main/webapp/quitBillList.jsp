...
<form id="pagerForm" method="post" action="quit/list">
	<input type="hidden" name="pageNum" value="1" />
	<input type="hidden" name="no" value="${no}" />
</form>
<div class="pageHeader">
	<form onsubmit="return navTabSearch(this);" action="quit/list" method="post" rel=”pagerForm”>
	<div class="searchBar">
...
<div class="pageContent">
	<div class="panelBar">
		<ul class="toolBar">
			<li><a class="add" href="quit/add" rel="quitAdd" target="navTab"><span>添加</span></a></li>
			<li class="line">line</li>
			<li><a class="delete" href="quit/delete/{e_id}" target="ajaxTodo" title="确定要删除吗?"><span>删除</span></a></li>
			<li class="line">line</li>
			<li><a class="edit" href="quit/edit/{e_id}" target="navTab"><span>查看表单</span></a></li>
			<li class="line">line</li>
		...
		<c:forEach items="${pages.data}" var="e">
			<tr target="e_id" rel="${e.id}">
				...
				<td><c:if test="${e.status==2}"> <a href="flow/image/${e.flowId}" target="navTab">查看办理进度</a></c:if>
					...
					<a href="quit/deal/${e.id}" target="ajaxTodo">办理</a> &nbsp;|&nbsp;
					<a href="quit/edit/${e.id}" target="navTab">修改</a> &nbsp;|&nbsp;
					<a href="quit/delete/${e.id}" target="ajaxTodo">作废</a>
					</c:if>
				</td>
			</tr>
			</c:forEach>
